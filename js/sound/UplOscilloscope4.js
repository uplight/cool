...
(function (upl) {
    ...
    var Oscilloscope = (function () {
        function Oscilloscope(prefix) {
            ...
            this.select = view.find('[data-id=input_select]:first').on('change', null, function (evt) { return _this.onInputChanged(evt); });
            ...
            if (navigator.getUserMedia)
                setTimeout(function () { return _this.getInputsList(); }, 200);
            else
                alert('browser doest support getUserMedia');
            this.speed = document.getElementById(prefix + '-speed');
            this.maxValue = document.getElementById(prefix + '-maxval');
            this.oscAnalyser = new OscRecorder(prefix);
            this.btnStartStop = view.find('[data-id=start_stop]:first').on('click', null, function (evt) { return _this.onStartStop(evt); });
            ...
            this.btnPrint = view.find('[data-id=print]:first').on('click', null, function (evt) { return _this.onPrintClick(evt); }).attr('disabled', true);
            this.btnClear = view.find('[data-id=clear]:first').on('click', null, function (evt) { return _this.onClearClick(evt); }).attr('disabled', true);
            this.sensitivityV = view.find('[data-id=sensitivity]:first').on('change', null, function (evt) { return _this.onSensitivityChanged(evt); });
        ...
        Oscilloscope.prototype.getInputsList = function () {
            var _this = this;
            MediaStreamTrack.getSources(function (info) { return _this.gotSources(info); });
        ...
        Oscilloscope.prototype.getUserMedia = function (constraints) {
            var _this = this;
            navigator.getUserMedia(constraints, function (stream) { return _this.onConnect(stream); }, function (e) {
                alert('Error getting audio');
                console.log(e);
            });
        };
        ...
        Oscilloscope.prototype.onConnect = function (stream) {
            ...
            this.jsProcessor.onaudioprocess = function (evt) { return _this.onProcess(evt); };
        ...
        */
        Oscilloscope.prototype.onProcess = function (evt) {
            ...
            for (var i = 1, n = size; i < n; i++) {
                var val = data[i];
                g.lineTo(step * i, val);
                if ((val < (128 - max)) || (val > (128 + max)))
                    max = Math.abs(128 - val);
                if (sense == 0)
                    continue;
    ...
    var OscRecorder = (function () {
        ...
        OscRecorder.prototype.addData = function (ar, timestamp) {
            ...
            this.range.val(l);
            setTimeout(function () { return _this.drawArrayScreen(ar, timestamp); }, 20);
            if (l >= this.length)
                this.gotFull();
            return l;
        };
...
//# sourceMappingURL=UplOscilloscope4.js.map